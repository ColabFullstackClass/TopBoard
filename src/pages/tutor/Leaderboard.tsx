...
export const StatBox: React.FC<StatBoxProps> = ({ label, value, highlight, icon }) => {
  return (
...
export const StudentList: React.FC<StudentListProps> = ({ students, onAdd }) => {
  ...
  return (
    <div className="bg-white rounded-[16px] border border-[#F3F4F6] overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#F3F4F6]">
        <span className="text-[#8E91A1] text-[12px] font-medium">Student List</span>
        {onAdd && (
          <button
            type="button"
      ...
      {isAdding && (
        <form onSubmit={handleSubmit} className="px-4 py-3 border-b border-[#F3F4F6] grid grid-cols-1 sm:grid-cols-4 gap-2">
          <input
          ...
          <input
            ...
            inputMode="decimal"
          />
          <button type="submit" className="bg-[#1E1F27] text-white rounded px-3 py-2 text-sm">Add</button>
        </form>
      )}
      <ul>
...
const initialStudents: Student[] = [
  ...
  { id: "2", name: "Emmanuel Senior Dev.", code: "PRAC-02-02", scorePercent: 92.1 },
  { id: "3", name: "Murna Smith", code: "PRAC-02-03", scorePercent: 86.2 },
  { id: "4", name: "Ade Nikky", code: "PRAC-02-04", scorePercent: 88.3 },
  { id: "5", name: "Adeswalla Chelsea", code: "PRAC-02-05", scorePercent: 84.9 },
...
const Leaderboard: React.FC = () => {
  ...
  const average = students.length
    ? (students.reduce((s, c) => s + c.scorePercent, 0) / students.length).toFixed(1)
    : "0.0";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {stats.map((s, idx) => (
          <StatBox
            ...
            icon={idx === 3 ? <FaTrophy className="text-[#F59E0B]" /> : undefined}
          />
        ))}
      </div>
